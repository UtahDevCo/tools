...
export async function handleVerify(c: Context<{ Bindings: Env }>) {
  try {
    ...
    if (!token) {
      return c.json({ error: { code: 'MISSING_TOKEN', message: 'Missing token' } }, 400);
    }
    ...
    const refreshToken = await generateRefreshToken(c.env.JWT_PRIVATE_KEY, user, refreshTokenId, appId);
