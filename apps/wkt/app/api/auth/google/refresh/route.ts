...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!refreshToken) {
      refreshToken = cookieStore.get("wkt_refresh_token")?.value;
    }
    ...
    if (!email) {
      cookieStore.set("wkt_access_token", credentials.access_token, {
        httpOnly: true,
      ...
      cookieStore.set("wkt_token_expiry", expiresAt.toString(), {
        httpOnly: false,
        secure: env.NODE_ENV === "production",
        sameSite: "lax",
        path: "/",
      ...
      if (credentials.refresh_token) {
        cookieStore.set("wkt_refresh_token", credentials.refresh_token, {
          httpOnly: true,
          secure: env.NODE_ENV === "production",
          sameSite: "lax",
          path: "/",
