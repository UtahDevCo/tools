...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (mode === "primary") {
      ...
      cookieStore.set("wkt_access_token", tokenData.accessToken, {
        httpOnly: true,
      ...
      if (tokenData.idToken) {
        cookieStore.set("wkt_id_token", tokenData.idToken, {
      ...
      cookieStore.set("wkt_user_data", JSON.stringify(userData), {
        httpOnly: false,
        secure: env.NODE_ENV === "production",
        sameSite: "lax",
        path: "/",
      ...
      // Store token expiry
      cookieStore.set("wkt_token_expiry", tokenData.expiresAt.toString(), {
        httpOnly: false,
        secure: env.NODE_ENV === "production",
        sameSite: "lax",
        path: "/",
      ...
      if (tokenData.refreshToken) {
        cookieStore.set("wkt_refresh_token", tokenData.refreshToken, {
          httpOnly: true,
          secure: env.NODE_ENV === "production",
          sameSite: "lax",
          path: "/",
      ...
      // For secondary account, redirect to settings
