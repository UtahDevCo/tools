  ...
  if (!isAuthenticated || !user) {
    return (
      <Button
        variant="ghost"
        size="sm"
        className="h-10 gap-2 rounded-full bg-brand px-4 text-white hover:bg-brand/90"
        onClick={() => signIn()}
      >
        Sign in
      </Button>
    ...
    <Popover>
      <PopoverTrigger asChild>
        <button className="rounded-full focus:outline-none focus:ring-2 focus:ring-brand focus:ring-offset-2">
          <Avatar className="size-10 bg-zinc-800 text-white cursor-pointer hover:ring-2 hover:ring-brand">
            {user.photoURL && (
              <AvatarImage
                src={user.photoURL}
